...
export default function ManageAchievements() {
  ...
  const fetchAchievements = async () => {
    try {
      const response = await fetch("/api/getAchievementnews");
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "This achievement will be deleted permanently!",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`/api/deleteAchievementsnews?id=${id}`, { method: "DELETE" });
        if (response.ok) {
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 p-8 space-y-6">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">Achievements List</h2>
                ...
                {achievementList.length > 0 ? (
                  achievementList.map((achievement) => (
                    <tr key={achievement.id} className="text-center">
                      ...
                      <td className="border border-gray-300 px-4 py-2">{achievement.des}</td>
                      <td className="border border-gray-300 px-4 py-2">
                        {achievement.file_path ? (
                          <button 
                            onClick={() => handleViewFile(achievement.file_path)} 
                      ...
                      <td className="border border-gray-300 px-4 py-2">
                        <button onClick={() => handleDelete(achievement.id)} className="text-red-500 hover:text-red-700">
                          <Trash2 size={30} />
                        </button>
                      </td>
                    </tr>
                  ...
                  <tr>
                    <td colSpan="4" className="text-center py-4 text-gray-500">No achievements available.</td>
                  </tr>
                )}
              </tbody>
            </table>
