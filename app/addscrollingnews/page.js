...
export default function AddNews() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("/api/addScrollingnews", {
        method: "POST",
        body: formData,
      });
      ...
      if (response.ok) {
        setTitle("");
        setFile(null);
        router.push("/managescrollingnews"); // Redirect to manage news after adding
      }
    } catch (error) {
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 p-8">
          <div className="max-w-lg mx-auto bg-white p-6 rounded shadow">
            <h2 className="text-xl font-bold mb-4">Add Scrollingnews</h2>
