...
export default function AddNews() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("/api/addAchievementsnews", {
        method: "POST",
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        setDes("");
        setFile(null);
        router.push("/manageachievementsnews"); // Redirect to manage achievements after adding
      }
    } catch (error) {
      setMessage("Error uploading achievement");
    }
  };
